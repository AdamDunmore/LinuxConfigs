...
{
  config = lib.mkIf config.adam.display_managers.greetd {
    services.greetd = {
        enable = true;
        settings = {
            default_session = {
              command = "${pkgs.cage}/bin/cage -s -- ${pkgs.ags}/bin/ags --config ${pkgs.writeText "greetd-ags-js"  ''
    ...
    const Greeter_Session_Selector = Widget.Switch({
        on_activate: function({ active }){
            if(active){
                ...
                Error1.label = "${config.adam.wm.window_managers.default}";
